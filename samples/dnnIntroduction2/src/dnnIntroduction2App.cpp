...
//
// From https://github.com/davisking/dlib/blob/master/examples/dnn_introduction2_ex.cpp
//
...
class dnnIntroductionApp2 : public App {
public:
...
void dnnIntroductionApp2::prepare(Settings* aSettings)
{
    aSettings->setTitle("dnn Introduction 2");
    aSettings->setWindowSize(ivec2(640, 480));
}

void dnnIntroductionApp2::setup()
{
    ...
    console() << "The pnet has " << pnet.num_layers << " layers in it." << endl;
    console() << pnet << endl;
    ...
    console() << "prelu param: "<< layer<7>(pnet).layer_details().get_initial_param_value() << endl;
    ...
    trainer.set_synchronization_file((getAssetPath("")/"mnist_resnet_sync").string(), std::chrono::seconds(100));
    ...
    net.clean();
    dlib::serialize((getAssetPath("")/"mnist_res_network.dat").string()) << net;
    ...
    dlib::deserialize((getAssetPath("")/"mnist_res_network.dat").string()) >> tnet;
...
void dnnIntroductionApp2::mouseDown( MouseEvent event )
{
}

void dnnIntroductionApp2::update()
{
}

void dnnIntroductionApp2::draw()
{
    gl::clear( Color( 0, 0, 0 ) );
}

CINDER_APP( dnnIntroductionApp2, RendererGl, &dnnIntroductionApp2::prepare )
