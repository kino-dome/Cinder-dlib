...
//
// From https://github.com/davisking/dlib/blob/master/examples/dnn_introduction_ex.cpp
//
...
class dnnIntroductionApp : public App {
  public:
...
void dnnIntroductionApp::prepare(Settings* aSettings)
{
    aSettings->setTitle("dnn Introduction");
    aSettings->setWindowSize(ivec2(640, 480));
}

void dnnIntroductionApp::setup()
{
    ...
    // the file exists.
    trainer.set_synchronization_file((getAssetPath("")/"mnist_sync").string(), std::chrono::seconds(20));
    ...
    net.clean();
    dlib::serialize((getAssetPath("")/"mnist_network.dat").string()) << net;
    ...
    dlib::net_to_xml(net, (getAssetPath("")/"lenet.xml").string());
...
void dnnIntroductionApp::mouseDown( MouseEvent event )
{
}

void dnnIntroductionApp::update()
{
}

void dnnIntroductionApp::draw()
{
	gl::clear( Color( 0, 0, 0 ) );
}

CINDER_APP( dnnIntroductionApp, RendererGl, &dnnIntroductionApp::prepare )
